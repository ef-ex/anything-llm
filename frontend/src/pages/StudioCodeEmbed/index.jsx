...
export default function StudioCodeEmbed() {
  ...
  return <StudioEmbedRouter />;
}

function StudioEmbedRouter() {
  const [searchParams] = useSearchParams();
  if (isStudioAskEmbed(searchParams)) {
    return <StudioAskEmbedChat />;
  }
  return <StudioCodeEmbedChat />;
}

function StudioAskEmbedChat() {
    ...
    async function ensureThreadRoute() {
      ...
      try {
        ...
        if (mainThreads.length > 0) {
          navigate(studioAskThreadPath(workspace.slug, mainThreads[0].slug), {
            replace: true,
          });
          return;
        }
        ...
        if (thread) {
          navigate(studioAskThreadPath(workspace.slug, thread.slug), {
            replace: true,
          });
          return;
        }
        console.warn("[vela] could not create initial ask thread:", error);
      } finally {
  ...
  if (!workspace) {
    return (
      <div className="w-screen h-[100dvh] bg-zinc-950 flex items-center justify-center p-6 text-center text-zinc-400 text-sm">
        This Ask workspace could not be loaded. Run the dev stack and open Ask
  ...
  return (
    <div className="w-screen h-[100dvh] overflow-hidden bg-zinc-950 light:bg-slate-50">
      <WorkspaceChatContainer
        loading={false}
        workspace={workspace}
        embedded
        hideStudioCodeRolePicker
        studioCodeResolvedRoleId={STUDIO_ASSISTANT_ROLE_ID}
      />
    </div>
  );
}
